...
@GetMapping("/hateoas/{id}")
    public ProductoDTO obtenerHATEOAS(@PathVariable Integer id) {
        ...
        dto.add(Link.of("http://localhost:8084/api/proxy/productos/" + dto.getId()).withSelfRel());
        dto.add(Link.of("http://localhost:8084/api/proxy/productos/" + dto.getId()).withRel("Modificar HATEOAS").withType("PUT"));
        dto.add(Link.of("http://localhost:8084/api/proxy/productos/" + dto.getId()).withRel("Eliminar HATEOAS").withType("DELETE"));
    ...
    public List<ProductoDTO> obtenerTodosHATEOAS() {
        ...
        for (ProductoDTO dto : lista) {
            ...
            //link HATEOAS para API Gateway "A mano"
            dto.add(Link.of("http://localhost:8084/api/proxy/productos").withRel("Get todos HATEOAS"));
            dto.add(Link.of("http://localhost:8084/api/proxy/productos/" + dto.getId()).withRel("Crear HATEOAS").withType("POST"));
        }
